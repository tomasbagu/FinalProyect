...
export default function AddMedicationScreen() {
  ...
  const handleAdd = async () => {
    if (!id || !name || selectedDays.length === 0) return;
    const ok = await assignMedication(id.toString(), {
      type,
      name,
      dailyDose,
          ...
          <View style={styles.daysRow}>
            {durationOptions.map(opt => (
              <TouchableOpacity
                ...
                style={[styles.dayButton, durationDays === opt.value && styles.dayButtonSelected]}
              >
                <Text style={{ color: durationDays === opt.value ? '#fff' : '#000' }}>{opt.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
